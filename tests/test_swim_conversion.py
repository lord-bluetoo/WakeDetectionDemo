...
def test_out_of_bounds_corners_are_preserved_by_default(tmp_path: Path) -> None:
    xml = tmp_path / "outside.xml"
    _write_xml(xml, cx=5)

    records, stats = convert_annotation(xml)

    assert min(records[0]) < 0
    assert stats.out_of_bounds_boxes == 1
    assert stats.clipped_boxes == 0


def test_out_of_bounds_corners_can_be_clipped_explicitly(tmp_path: Path) -> None:
    xml = tmp_path / "outside.xml"
    _write_xml(xml, cx=5)

    records, stats = convert_annotation(xml, clip_boxes=True)
